...
5. **Web 管理** — HTTP Basic Auth 保护的配置页面 + 工具箱
6. **管理员命令** — 通过短信远程执行 SMS:发送 和 RESET 命令（仅限管理员号码）
7. **号码黑名单** — 支持按号码过滤骚扰短信
...
├── web_handlers.h / .cpp # HTTP 请求处理器（配置页/工具箱/Ping/查询）
├── web_html.h / .cpp     # HTML 页面模板（原始字符串常量）
