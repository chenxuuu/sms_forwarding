...
Two threads: the Arduino **loop task** + one dedicated **push/email worker task**. Almost everything
runs on `loop()` (web server, modem AT/Serial1, SMS receive+forward-decision, keep-alive); the worker
(`pushWorkerTask` in `push.cpp`, started by `startPushWorker()` in `setup()`) runs ONLY the slow
WiFi/TLS sends — `processRetryQueue()` (HTTP push), `processEmailQueue()` (SMTP), `processTestPushQueue()`
— so a blocking TLS/SMTP send never freezes the web UI or SMS reception. The worker touches **only WiFi**
(global `smtp`/`ssl_client` are worker-exclusive); it must NEVER touch `Serial1`/modem, `inbox`,
`concatBuffer`, `pdu`, or `smsTotalCount`. Single worker ⇒ at most one TLS session at a time (heap peak
unchanged; `TLS_MIN_FREE_HEAP` precheck still guards OOM).

**Locking (keep it minimal, in `globals.h`):** `gWorkMux` guards the push/email/test job queues **and**
the worker's snapshot reads of `config.pushChannels[]`/`config.smtp*` (writers: `handleSave`/`handleImport`);
`gLogMux` guards the log ring buffer (`logCapture*` ↔ `handleLog`/`handleLogDownload`). **Rules:** never
hold a mutex during a network/AT call (lock only to claim/snapshot/free, unlock, then send); lock order is
`gWorkMux`→`gLogMux` only (never reverse — `logCapture` is a leaf, never takes `gWorkMux`); use `muxLock`/
`muxUnlock` (NULL-safe for pre-`initConcurrency()` early boot). `gSlowWorkBusy` (volatile) marks the email
in-flight window (email dequeues before sending) so `heapGuardTick`/daily-reboot don't restart mid-send.
Web handlers run *inside* `server.handleClient()` on the loop task, so loop-vs-loop accesses (e.g.
`forwardNow` reading config while `handleSave` writes it) are not races — only worker-vs-loop needs a lock.
`initConcurrency()` (creates the mutexes) runs first in `setup()`; `startPushWorker()` runs after
...
(AT+CEREG probe + auto power-cycle recovery), `processForwardQueue()` (fast: rule-eval + enqueue into the
worker's queues, no network), `heapGuardTick()` (low-heap orderly restart), `keepAliveTick()` (SIM
keep-alive), and a `yield()`. (Push/email/test sends moved off `loop()` onto the worker — the old
`slowWorkGraceActive()`/`FORWARD_WEB_GRACE_MS`/`PUSH_JOB_GAP_MS` throttling that made forwarding feel slow
is gone; the worker's single-task serialization is the natural rate limit.)
